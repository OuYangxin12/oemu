...
.PHONY: format
# CI pins these (see .github/workflows/ci.yml): formatting verdicts move between
# clang-format releases -- 18 and 21 disagree about the same source line -- so the
# version is chosen, not whatever the runner happens to carry. Match CI elsewhere:
...
format: ## Rewrite sources with clang-format
	@command -v $(CLANG_FORMAT) >/dev/null 2>&1 || \
		{ echo "$(CLANG_FORMAT) not found (apt install clang-format)"; exit 1; }
...
tidy: compile-db ## Run clang-tidy over the C sources
	@command -v $(CLANG_TIDY) >/dev/null 2>&1 || \
		{ echo "$(CLANG_TIDY) not found (apt install clang-tidy)"; exit 1; }
