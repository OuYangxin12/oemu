 ...
 * length. It once accused the emulator of stalling the second write and was itself
      at fault (see sys3), so treat an unexplained -EBADF here as this file's
      problem first. Build it
...
/*
 * AArch64 syscall ABI: the number in x8, args in x0..x5, the answer in x0 (and
 * x1 for a pair). The kernel is free to scratch x0..x5 and x8, so every one of
 * them that is not an explicit input/output has to be declared dead -- otherwise
 * the optimiser keeps a live value in, say, x3 across the trap and the next call
 * hands the kernel yesterday's arguments. This fixture once reported a "console
 * stall" that turned out to be exactly that: the fd argument had been folded into
 * the previous call's return value, so the guest was correctly answering -EBADF
 * to write(8, ...). Read the args out of the trace, not out of this file.
 */
